...
const SlideOutMenu: FunctionComponent<SlideOutMenuProps> = ({ onClose }) => {
  ...
  const [filterValues, setFilterValues] = useState({
    ...
    selectedJobFunctions:
      searchParams.get("jobFunctions")?.split(",").filter(Boolean) ||
      ([] as string[]),
    jobSubfunctions:
      searchParams.get("jobSubfunctions")?.split(",").filter(Boolean) ||
      ([] as string[]),
    salaryRange: { min: "", max: "" },
    EmploymentType:
  ...
  const handleJobSubfunctionChange = useCallback(
  ...
  const handleApplyFilters = () => {
    ...
    if (filterValues.selectedJobFunctions.length > 0) {
      queryParams.set(
        "jobFunctions",
        filterValues.selectedJobFunctions.join(",")
      );
    } else {
    ...
    if (filterValues.jobSubfunctions.length > 0) {
      queryParams.set(
        "jobSubfunctions",
        filterValues.jobSubfunctions.join(",")
      );
    } else {
